...
- **What we decided**: brief answers stay in the browser draft only; the selection schema and Netlify validator are unchanged. `presetId` records the closest existing look.
- **Where we left off**: all checks green (brief 8, Node 34, Python 27, browser 24/24, axe clean on all 11 steps, quality.sh). Not committed or PR'd; awaiting Britt's review.

## Open Items
- [ ] BeSpoke brief/UI rework (2026-09-23 worktree `bespoke-wizard-ui-review-c1971a`): Britt to review, then commit/PR. Brief answers are not in the shared payload, so they reset on another computer; add an optional `brief` field to the schema + Netlify validator only if Britt wants them to travel.
