...
- [ ] **BeSpoke autosave + close warning** built 2026-09-23 on branch `claude/bespoke-autosave` (not yet committed/merged): shared save on step change (1.5 s) and after 30 s idle; beforeunload prompt while changes are unshared; paused after opening a backup/older version until a manual Save; conflicts still stop it. 24 browser scenarios pass; quality.sh green. Live only after merge + Pages deploy.
- [ ] Check the BeSpoke service PAT (`BESPOKE_GITHUB_TOKEN`) expiry date — Britt's GitHub account; Save/Open stop when it expires. Consider a scheduled expiry reminder.
